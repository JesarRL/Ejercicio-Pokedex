    ...
    for (const pokemon of arrayPokemones){
        ...
        personaje.stats.forEach(dataStats => {
            if (dataStats.stat.name == 'hp') {
                ...
                barrasEstadisticas += `<div><strong> Defensa: </strong><div class="progress mb-2 fs-5 h-25" role="progressbar" aria-label="Basic example" aria-valuenow="${dataStats.base_stat}" aria-valuemin="0" aria-valuemax="100">
                <div class="progress-bar bg-warning" style="width: ${dataStats.base_stat}%">${dataStats.base_stat}/100</div></div>`
            }
            else if (dataStats.stat.name == 'speed') {
                barrasEstadisticas += `<div><strong> Velocidad: </strong><div class="progress mb-2 fs-5 h-25" role="progressbar" aria-label="Basic example" aria-valuenow="${dataStats.base_stat}" aria-valuemin="0" aria-valuemax="100">
